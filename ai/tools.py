...
def get_tools_definitions():
    ...
    return [
        ...
        },
        {
            ...
            "description": (
                "Подсчитать сделки, которые ПРОШЛИ ЧЕРЕЗ определённую стадию воронки за период. "
                "В отличие от get_deals (который смотрит на ТЕКУЩУЮ стадию), этот инструмент "
                "учитывает историю — даже если сделка сейчас уже на следующей стадии или "
                "провалена. ИСПОЛЬЗУЙ для подсчёта замеров, продаж за период, конверсий."
            ),
            "input_schema": {
                "type": "object",
                "properties": {
